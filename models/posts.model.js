...
class PostsModel extends Model {
    ...
    async fetchMessages(){
        ...
        try{
            let fetch_message_query = Mysql.format(`
                SELECT messages.id, messages.user_id, messages.message, messages.created_at, CONCAT(users.first_name, " ", users.last_name) AS name,
                (
                    SELECT 
    ...
    async addMessage(user_data){
        let response_data = {status: false, result: {}, error: null};

        try{
            let add_message_query = Mysql.format(`INSERT INTO messages SET ?, created_at=NOW()`, [user_data]);
            ...
            response_data.error = error;
            response_data.message = "Error encountered while adding message.";
        }
         
        return response_data;
    }

    async addComment(user_data){
        let response_data = {status: false, result: {}, error: null};

        try{
            let add_comment_query = Mysql.format(`INSERT INTO comments SET ?, created_at=NOW()`, [user_data]);
            ...
            response_data.error = error;
            response_data.message = "Error encountered while adding comment.";
        }
        ...
        try{
            ...
            if(fetch_message.status){
                if(fetch_message.result.user_id === params.user_id){
                    ...
                    response_data.message = "You can't delete this message";
                }
            }
            else{
    ...
    async fetchMessage(message_id){
        ...
        try{
            ...
            if(fetch_message_result){
                ...
                response_data.message = "Failed to fetch message.";
            }
        }
        catch(error){
            response_data.error = error;
        ...
        try{
            ...
            if(fetch_comment.status){
                if(fetch_comment.result.user_id === params.user_id){
                    ...
                    response_data.message = "You can't delete this comment";
                }
            }
            else{
            ...
            response_data.error = error;
            response_data.message = "Error encountered while deleting comment.";
        }
